...
public class HexCell : MonoBehaviour
{
    ...
    {
        if (!roads[(int) direction] && !HasRiverThroughEdge(direction) && !IsSpecial && !GetNeighbour(direction).IsSpecial && GetElevationDifference(direction) <= 1)
        {
    ...
    void ValidateRivers ()
    {
        if (
            hasOutgoingRiver &&
            !IsValidRiverDestination(GetNeighbour(outgoingRiver))
        )
        {
            RemoveOutgoingRiver();
        }
        if (
            hasIncomingRiver &&
            !GetNeighbour(incomingRiver).IsValidRiverDestination(this)
        )
        {
            RemoveIncomingRiver();
        }
